...
def generate_launch_description():
    ...
    # Configure GAZEBO_MODEL_PATH to include go2_config models
    go2_models_path = ""
    try:
        go2_config_share = launch_ros.substitutions.FindPackageShare(package="go2_config").find("go2_config")
        go2_models_path = os.path.join(go2_config_share, "models")
    except:
        pass
    
    # Get existing GAZEBO_MODEL_PATH and append go2 models
    existing_model_path = os.environ.get('GAZEBO_MODEL_PATH', '')
    if go2_models_path and go2_models_path not in existing_model_path:
        if existing_model_path:
            new_model_path = f"{existing_model_path}:{go2_models_path}"
        else:
            new_model_path = go2_models_path
    else:
        new_model_path = existing_model_path if existing_model_path else go2_models_path
